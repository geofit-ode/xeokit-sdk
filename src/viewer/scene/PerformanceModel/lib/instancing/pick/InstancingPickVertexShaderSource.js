 ...
 * @private
 */
const InstancingPickVertexShaderSource = function (scene) {
...
function buildVertex(scene) {
    ...
    src.push("// Instancing vertex picking vertex shader");
    ...
    src.push("      gl_PointSize = 30.0;");  // TODO: Make configurable
...
function buildFragment(scene) {
    ...
    src.push("// Instancing vertex picking fragment shader");
    ...
    src.push("uniform vec4 viewportAndClipPlanes;");
    ...
    src.push("vec4 packNormalizedFloat(const in float depth) {");
    ...
    // Render normalized view-space X, Y or Z

    src.push("  if (axis == 0) {"); // X

    // src.push("      float viewportWidth = viewportAndClipPlanes[0];");
    // src.push("      float viewportHeight = viewportAndClipPlanes[1];");
    //
    // //src.push("      float zNormalizedDepth = abs((zNear + vViewPosition.z) / (zFar - zNear));");
    // src.push("      gl_FragColor = packNormalizedFloat(zNormalizedDepth);");

    src.push("  } else if (axis == 1) {"); // Y
    ...
    src.push("      float zNear = viewportAndClipPlanes[2];");
    src.push("      float zFar  = viewportAndClipPlanes[3];");
...
export {InstancingPickVertexShaderSource};